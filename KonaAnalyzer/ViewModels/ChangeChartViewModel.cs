    ...
    public class ChangeChartViewModel : BaseViewModel
    {
        ...
        public ChangeChartViewModel(ILocationSource locationSource, ICovidSource covidSource)
        {
            ...
            this.WhenAnyValue(x => x.State).Subscribe(x =>
           {
               ...
               //Update(x, "All", StartDate, EndDate);
            ...
            this.WhenAnyValue(x => x.County).Subscribe(x => { Update(State, x, StartDate, EndDate); }, OnException);
            this.WhenAnyValue(x => x.StartDate).Subscribe(x => { Update(State, County, x, EndDate); }, OnException);
            this.WhenAnyValue(x => x.EndDate).Subscribe(x => { Update(State, County, StartDate, x); }, OnException);
            this.WhenAnyValue(x => x.DataType).Subscribe(x => { Update(State, County, StartDate, EndDate); }, OnException);
        ...
        private void Update(string state, string county, DateTime startDay, DateTime endDay)
        {
            ...
            Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    var change = DataStore.CountyChanges(state, county, startDay, endDay);
                    //State = state;
                    //County = county;
